...
?>
    ...
    ?>
        <label class="col-sm-1 control-label no-padding-right"><?php echo $item_customization_value['title'];?></label>
        <div class="col-xs-6 col-sm-6">
        <select data-title="<?php echo $item_customization_value['title'];?>" class="chosen-select tag-input-style customization_type customization_type_select need_check" data-id="<?php echo $item_customization_value['id'];?>" id="<?php echo htmlspecialchars('customization_' . $item_customization_value['id']);?>" <?php if($item_customization_value['extra']['multi-selectable']){echo 'multiple="multiple"';}?> data-placeholder="请选择标签">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        if(PAGE_TYPE==0) {
            setTimeout(function () {
                $('.customization_type_select').val(null).trigger('chosen:updated');
            });
        }
    });
</script>