...
<div class="col-xs-6 col-sm-6 image-attachment">
...
function initUploader() {
    ...
    uploader.on('fileQueued', function(file) {
        var $li = $('<a id="' + file.id + '" data-rel="colorbox" style="position:relative;float:left;border: 1px solid #ddd;border-radius: 4px;margin-right: 15px;padding: 5px;">' + '<img>' + '<div class="glyphicon glyphicon-trash red del-button" style="  position: absolute;right: 10px;top: 10px;cursor: pointer;"></div>' + '</a>'),
            $img = $li.find('img');
        // $list为容器jQuery实例
        $('#imageList').append($li);
        ifUp = 0;
...
function load_exists() {
    ...
    $(images).each(function(idx, item) {
        ...
        var $li = $('<a href="' + item['url'] + '" id="WU_FILE_' + item.id + '" data-rel="colorbox" style="position:relative;float:left;border: 1px solid #ddd;border-radius: 4px;margin-right: 15px;padding: 5px;">' + '<img style="width:150px;height:150px;">' + '<div class="glyphicon glyphicon-trash red del-button" style="  position: absolute;right: 10px;top: 10px;cursor: pointer;"></div>' + '</div>'),
            $img = $li.find('img');
        // $list为容器jQuery实例
        $('#imageList').append($li);
        // 创建缩略图
